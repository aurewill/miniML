...
let repl () =
  ...
  while true do
    (try
        ...
        (* EVALuate it *)
        (* let res_t = Ev.eval_t exp env in
        ...
        (* match res_t, res_s, res_d, res_l with
        | Val resexp_t, Val resexp_s, Val resexp_d, Val resexp_l ->
          (printf "--> %s\n" (Ex.exp_to_abstract_string resexp_t);
           printf "s=> %s\n" (Ex.exp_to_concrete_string resexp_s);
           printf "d=> %s\n" (Ex.exp_to_concrete_string resexp_d);
           printf "l=> %s\n" (Ex.exp_to_concrete_string resexp_l))
        ...
        | Closure (res) -> printf "--> %s\n" (Ev.Env.value_to_string (Closure (res)))
        | _ -> failwith "not handling other cases yet"
